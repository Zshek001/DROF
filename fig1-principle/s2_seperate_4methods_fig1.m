...
[Z_fit1, Z_fit2, Z_noe, Z_guan, Z_amide] = fitting_DROF_demo(offs, zspec, FitParam, multistart_N);
...
function [Zamide_DZ_vec, Zguan_DZ_vec, Znoe_DZ_vec, fit_para] = fitting_MPLF(offs, zspec_vec, multistart_N)
% OUTPUT
...
%       Zi (1), Water (2-4), Amide (5-7), NOE (8-10), MT (11-13), Guan (14-16)
    ...
    for i = 1:Npixel
        ...
        Z_MT = lorentzMultipool([par(1:10),par(14:end)], offs) - Z_fit; 
        Z_amide = lorentzMultipool([par(1:4), par(8:end)], offs) - Z_fit; % amide
        Z_noe = lorentzMultipool([par(1:7),par(11:end)], offs) - Z_fit; 
        Z_guan = lorentzMultipool(par(1:13), offs) - Z_fit; 
...
function [Z_fit1, Z_fit2, Z_noe, Z_guan, Z_amide] = fitting_DROF_demo(offs, zspec_vec, FitParam, multistart_N)
...
function [Z_fit1, Z_fit2, Z_noe, Z_guan, Z_amide] = fitting_QDROF_demo(offs, zspec_vec, FitParam, multistart_N)
% use steady-state signal equation: "Z = (1 - cos2thet*R1/R1rho).* exp(-R1rho * tsat) + cos2thet * R1/R1rho"
% OUTPUT
%   Zamide_DZ_vec: [1,Npixel]
%   Zguan_DZ_vec: [1,Npixel]
...
%       Zi (1), Water (2-4), Amide (5-7), NOE (8-10), MT (11-13), Guan (14-16)
    ...
    % % saturation settings
    % pulse1_pwr in uT
    % pulse1_dur pulse duration in s
    % gamma_hz 
    % B0 
    % FitParam.R1 = 1; % no T1 correction
    % FitParam.satpwr = pulse1_pwr; % saturation power (uT)
